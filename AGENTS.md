...
- What must never enter the repository: private documents, screenshots of private files, temporary design-tool URLs, credentials, financing material, invented marks, mark files or masters, product visuals without a recorded owner authorization. `node scripts/validate.mjs --only forbidden` enforces the list in `scripts/validate/forbidden.json`; `--only assets` and `--only rasters` prove the provenance and the pixel contracts; `--only branded` pins the twelve slide backgrounds that depict the official UP! box under decision 0012 (their prompts, hashes, reference, safe zones, contact sheets and the trademark wording). The mark on those backgrounds is not licensed for extraction; never crop it out or reuse it.
