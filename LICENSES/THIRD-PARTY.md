...
| Onboarding illustrations `get-started-1.png` to `get-started-4.png` (`assets/slides/onboarding/`) | Copied byte for byte from the mobile application bundle (`assets/image/`, commit `8d8f9f8c`, version 1.29.0) | Project assets of the shipped app, republished at the product owner's request (decision 0011). The mobile repository's LICENSE is the Apache License 2.0, copyright 2023 LUKSO Blockchain GmbH; the files carry no separate notice and their illustrator or generator is not recorded. Observed shipped art, not new work; no licence is added by this repository (OPEN-04). |
| App screen exports (`assets/screenshots/mobile-app/`) | Exported from the Mobile app Updates design file at the product owner's request (decision 0011) | Rights stay with the product owner; marks shown in the screens stay all rights reserved. Documentation and presentation use only; not covered by the repository's Apache-2.0 grant. |
