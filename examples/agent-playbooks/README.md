...
Expected outcome: a background from `assets/generated/backgrounds/slides-v2/` in the dark register (one family per section; `identity-network-dark.png` or `identity-orbits-dark.png` for this topic; each shows the official UP! box, which stays inside the image and is never cropped out as a mark), copy inside the family's measured safe zone from `assets/generated/backgrounds/README.md` in `type.deck.h2` and `type.deck.body` with one accent phrase, a transparent screen from `assets/screenshots/mobile-app/` placed at 299 by 634 outside the safe zone and clear of the boxes (never an old capture, never the exploratory screens without a label), alt text per file, no counts or prices, and no new raster committed without a provenance entry.

## 8. Add background variations

Prompt: "Generate two more slide backgrounds in the identity-orbits family."

Expected outcome: first a check whether the variation may show the UP! box. The pinned prompts show it under decision 0012, which covers the twelve pinned files only; a new branded variation needs its own recorded owner authorization and decision record before generation, otherwise the prompts drop the box and the reference and follow the generic IB-12 frame (background only, blank objects, nothing that reads as a word or mark, no people, no devices, both registers). Then: prompts derived from the family's entry in `assets/generated/backgrounds/slides-v2/PROMPTS.source.json`, the native files added to `slides-v2/` as `<family>-light.png` and `<family>-dark.png`, `node scripts/inspect-png.mjs <file> --zone x,y,w,h` used to record the hash, dimensions, content-credentials time stamp and a safe zone that measures light or dark enough and quiet (recorded with its measured figures), entries added to `assets/generated/PROVENANCE.json` and to the prompt file with `status: "proposed"` (and, for an authorized branded file, the generation inputs and `embeddedMarks`), the set's count and family list raised, the contact sheets recomposed with `node scripts/compose-previews.mjs` and their placements and hashes recorded, `scripts/validate/branded-backgrounds.json` extended for a branded file, and `node scripts/validate.mjs --only assets`, `--only rasters`, `--only branded` and `npm test` passing.
