...
Prompt: "Make a 16:9 section slide about multichain profiles with a dark background."

Expected outcome: an expressive background from `assets/generated/backgrounds/slides-v2/` in the dark register (one family per section; `identity-network-dark.png` or `identity-orbits-dark.png` for this topic), copy inside the compact measured safe zone in `type.deck.h2` and `type.deck.body` with one accent phrase, the official UP! cubes left inside the whole image and never cropped out as a mark, alt text per file, no counts or prices, and no new raster committed without provenance. Slides-v2 remains the campaign, hero, section-divider and closing family.

## 8. Add background variations

Prompt: "Generate two more slide backgrounds in the identity-orbits family."

Expected outcome: first a check whether the variation may show the UP! box. The expressive authorization is decision 0012 and decision 0013 for exactly the twelve slides-v2 scenes and their two overview sheets; decision 0014 separately covers exactly the twelve ambient scenes and their two overview sheets. A new branded variation needs its own recorded owner authorization and decision before generation. Because identity orbits is expressive, derive both registers from its pinned slides-v2 prompt and keep the solid, closed and sealed container cube geometry plus its Avoid line; do not import the ambient density or threshold contract into IB-12. Record native files, prompts, ids, public relative references, hashes, content-credentials timestamps, visual cube counts and measured pair zone; update the slides-v2 set and expressive contact sheets only; extend `branded-backgrounds.json`; then run `--only assets`, `--only rasters`, `--only branded`, `--only ambient` and `npm test`. The ambient lock must still pass unchanged.

## 9. Build a content-heavy deck slide

Prompt: "Make a dark 16:9 product slide with two app screens, a short explanation and a chart."

Expected outcome: an IB-13 dark pair member from `assets/generated/backgrounds/slides-v3-ambient/`, selected so its large measured rectangle fits both copy and screens; heading and body in the deck roles, the chart and owner-authorized transparent screens inside the recorded zone and clear of the one or two edge cubes; white copy on a zone measuring at most 36 with deviation at most 12; no cube cropped or reused as a mark. Ambient is subordinate infrastructure for body copy, charts and screens, remains proposed, and is not described as a hero or a replacement for slides-v2. Decision 0014 covers only the twelve pinned ambient scenes and two overview sheets and grants no extraction or trademark licence.
