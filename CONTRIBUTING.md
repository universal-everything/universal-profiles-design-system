...
| Asset | Follow a brief in `imagery/briefs.md`; add or update the directory's `PROVENANCE.json` (method, tool or model, prompt, hash, dimensions, licence; register and safe zone for backgrounds; node id, source path or composition inputs plus status and authorization for observed material); `node scripts/inspect-png.mjs <file>` prints the facts; run `node scripts/validate.mjs --only assets` and `--only rasters`; no rasters with unrecorded rights and no product visuals without a recorded owner authorization |
