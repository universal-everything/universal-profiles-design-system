...
- **Families delivered (light and dark each), every file showing one or more official UP! boxes:** identity orbits (orbit system around one large and two smaller boxes on the right third, left 52 percent text-safe); glass profile stack (layered blank card planes on the left third carrying one large and two smaller boxes, right 50 percent text-safe); address ribbons (translucent ribbons inspired by address-derived colour signatures, lower left to upper right, carrying four boxes, calm centre-left band); modular constellation (blank cubes, spheres and tiles with one large and four smaller boxes along the lower-right edge, upper-left 58 percent text-safe); identity network (five boxes on orbital paths near the corners and edges, centre column calm); iridescent horizon (mist and low wave forms across the bottom quarter with three boxes at the lower right, upper 65 percent text-safe).
- **Branded generation (the delivered set):** each file is an image edit of the earlier generic file of the same name (Image 1, kept for composition, register and text-safe zone) with the official 3D UP! box as the second input (Image 2: the Universal Profile Board, file key `pAffzgmq6sjbWkyWAjnYmM`, node `1131:25142`, Box 5a; observed; a private input cited by hash, never published). Every prompt asks each box to keep the exact rounded periwinkle 3D form and the clearly readable white "UP!" glyph of the reference (the identity-orbits pair adds "no invented logo variations") and asks for "at least three" to "at least five" boxes per family. This is owner-authorized for the twelve pinned files only; a further branded variation needs its own recorded authorization before it is generated for the repository.
- **Constraints (in substance in every prompt, in the prompts' own words rather than this sentence; the exact prompts are in `PROMPTS.source.json`):** the only permitted text or mark is the UP! mark on the official 3D UP! boxes (every prompt states that only exact “UP!” branding may appear, beside "no other words, letters, numbers" and an exclusion list); the exclusion lists name UI screenshots, people, devices, coins, maps, grids and watermarks in all twelve prompts, logos in ten (the two identity-orbits prompts omit the word), blockchain symbols in ten (the two glass-profile-stack prompts omit it) and hands in the two identity-orbits prompts; support objects (card planes, tiles, cubes, spheres) blank wherever a family has them (glass profile stack, modular constellation); 16:9 landscape in all twelve, fully opaque with transparency excluded in ten (the identity-orbits prompts ask for the landscape and clean negative space); the named text-safe area preserved and no dominant pink in all twelve. Generic variations without an authorization keep the earlier frame instead: background only, all objects blank, nothing that reads as a word, letter or mark.
...
- **Status:** delivered (twelve files, the branded replacements of 2026-09-15) under `../assets/generated/backgrounds/slides-v2/` with the verbatim prompts, generation ids and former-file hashes in `PROMPTS.source.json` there and in the provenance record, pinned in `scripts/validate/branded-backgrounds.json`; proposed, not approved brand imagery; the depicted box is an observed mark under an owner-authorized publication and use decision, not brand approval, and the UP! mark stays its owner's trademark (no extraction or standalone reuse). New variations: copy a family prompt or write a new family inside the shared frame, generate both registers, record provenance, measure the safe zone, recompose the contact sheets with `scripts/compose-previews.mjs`; see `../assets/generated/backgrounds/README.md`.
